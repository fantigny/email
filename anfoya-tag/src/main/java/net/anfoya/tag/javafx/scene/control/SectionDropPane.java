...
public class SectionDropPane<S extends SimpleSection, T extends SimpleTag> extends GridPane {
	protected static final DataFormat DND_SECTION_DATA_FORMAT = new DataFormat(SimpleSection.class.getName());

	private final TagService<S, T> tagService;

	public SectionDropPane(final TagService<S, T> tagService) {
	...
	private void remove(final S section) {
		final Alert alertDialog = new Alert(AlertType.ERROR);
		alertDialog.setTitle("Create new section");
		alertDialog.setHeaderText("Section name is too short: ");
		alertDialog.setContentText("Section name should be a least 3 letters long.");
		alertDialog.showAndWait();
		try {
